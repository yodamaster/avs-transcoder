...
typedef struct tag_mpeg2_dec_create_t
{
  ...
  byte  *pBitstream;    /* [in] n* 2048 byte buffer */
} mpeg2_dec_create_t;
...
typedef struct tag_mpeg2_dec_frame_t
{
  int_32_t  version;
  int_16_t    frame_type;
  void_t    *bitstream;    /* [in] n * 2048 bytes bitstream (read from) */
  int_32_t  length;      /* [in] bitstream length */
  byte  *output;    /* [in] output image (written to) */
...
typedef struct tag_avs_enc_frame_t
{
  ...
  byte  *input;      /* [in] input image (read from) */
  byte  **inputfrm;
  ...
  void_t    *bitstream;    /* [in:opt] bitstream ptr (written to) */
