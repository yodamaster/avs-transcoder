...
#endif
  nOutBufPtr = 0;
  img->Frame_Total_Number_MB = img->width*img->height/256;
  return 0;
}
...
int_32_t c_avs_enc::avs_enc_destroy()
{
  ...
  memcpy(p_avs_enc_frame->bitstream, pOutBuffer, nOutBufPtr);
  p_avs_enc_frame->length = nOutBufPtr;
  nOutBufPtr = 0;*/
  /*
   * fclose(p_in);
   */  

  if(p_dec) fclose(p_dec);
  ...
  //释放全局申请的内存
  //free(p_avsCreate);  
  //free(p_avs_enc_frame->bitstream);
  //free(p_avs_enc_frame);  
  return 0;
}
...
int_32_t c_avs_enc::avs_enc_encode()
{
  ...
  total_encoded_frame = -1;
  gframe_no=current_encoded_frame; //20080331
  goprate=0; //20081108

  // xzhao 20080709 for DEBUG
  //if ((p_dec = fopen("dec.yuv", "r")) == 0)         // check if file exists
  //{
  //  if ((p_dec = fopen("dec.yuv", "ab")) == NULL)    // append new statistic at the end
  //  {
  //    printf("Error open file dec.yuv");
  //  }
  //}
  //else
  //{
  //  fclose (p_dec);
  //  if ((p_dec = fopen("dec.yuv", "ab")) == NULL)         // File exists, just open for appending
  //  {
  //    printf("Error open file dec.yuv");
  //  }
  //}
  ...
  for (i=0; i<dec_frm_num; i++)
  {
    ...
    if (i == 0)
      ...
      p_avs_enc_frame->type[0] = 2;
    else
      p_avs_enc_frame->type[0] = 3;


    p_avs_enc_frame->input = p_avs_enc_frame->inputfrm[i];
	
	//for (int j = 0; j <  input->img_height * input->img_width*3/2; j ++)
	//	fputc (p_avs_enc_frame->inputfrm[i][j], p_dec);
	
    avs_enc_frame(p_avs_enc_frame);
    gframe_no++;  // xzhao 20080331
  }
  memcpy((unsigned char*)p_avs_enc_frame->bitstream + p_avs_enc_frame->length, pORABS->buf, pORABS->iBytePosition);
  ...
  if(input->RCEnable)
  {
	  ...
	  //fprintf(p_dec, "GOP BITS: %d,       TARGET BITS: %d\n",goprate,(int_32_t) floor(dec_frm_num * input->bit_rate / input->fr + 0.5));
  }
#endif
...
void c_avs_enc::init_img()
{
  ...
  // xzhao 20081022
  //img->framerate = (int_32_t)FrameRate[input->frame_rate_code - 1];
  ...
  get_mem2Dint(&(img->ipredmode), img->width / B8_SIZE + 100, img->height / B8_SIZE + 100);  /* need two
                           * extra rows
                           * at right and
                           * bottom */
  ...
  img->mb_no_currSliceLastMB = (input->slice_row_nr != 0) ? min
    (
      input->slice_row_nr * img->img_width_in_mb - 1,
...
void c_avs_enc::report()
{
  ...
  if(Bframe_ctr != 0)
  {
    ...
    stat->bitrate_B = (stat->bit_ctr_B) *
      (float) (img->framerate / (input->successive_Bframe + 1)) *
  ...
  (
    ...
    (float) img->framerate /// (float) (input->successive_Bframe + 1)  //xzhao 20081227
  );
  if(input->hadamard)
...
void c_avs_enc::information_init()
{
  ...
  if(p_dec != NULL) printf(" Output YUV file                   : %s \n", input->ReconFile);
...
int_32_t c_avs_enc::init_global_buffers()
{
  ...
  /* xzhao { 2007.7.21 */
  imgY_org_buffer_restore = imgY_org_buffer;  /* to save the original allocated memory address, in case of
               * free */
...
int_32_t c_avs_enc::encode_IP_frame(avs_enc_frame_t *pFrame)
{
  ...
  TLS static int_32_t bFirstFrame = 1;
  /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
  ...
  // xzhao 20080331
  //picture_distance = img->number * (1 + input->successive_Bframe);
  ...
  if(input->RCEnable && img->type == INTRA_IMG)
  {
	  ...
	  n = input->GopLength;; // xzhao 20081108   Goplength和标准编码器中的IntraPeriod不同
	  np = 0;
	  nb = 0;
	  ...
	  for (int i=0; i<n; i++)
	  {
		  if (i == 0)
			  NULL;
		  else if (input->successive_Bframe == 0 || i % M == 1 || (i>n-input->successive_Bframe && n%M!=1))
              np++;
          else
			  nb++;
	  }
  ...
  img->number++;
  memcpy((unsigned char*)pFrame->bitstream + pFrame->length, pOutBuffer, nOutBufPtr);
...
int_32_t c_avs_enc::encode_B_frame(avs_enc_frame_t *pFrame)
{
  ...
  {
    // xzhao 20080331
    //picture_distance = (img->number - 1) * (input->successive_Bframe + 1) + img->b_frame_to_code;
