...
void DpbClean1( avs_dpb_t *p )
{
	free( p->frame );
}
static void DpbUpdate( avs_dpb_t *p, int is_forced )
{
	int i;
	int pos;
...
void DpbFlush1( avs_dpb_t *p )
{
	while (p->dpb.cnt > 0)
		DpbUpdate( p, true );
}

void DpbAdd1( avs_dpb_t *p, int poc, int is_idr )
{
	if (is_idr)
		DpbFlush1( p );
...
int DpbFrameOffset1( avs_dpb_t *p, int idx )
{
	if (idx >= p->cnt)
		return 0;
	if (p->frame[idx] < 0)
										...
										uint32_t header_size)
{
	...
	while (buffer_on < nal->buffer_on) {
		...
		start = buffer_on;
		payload_type = avs_read_sei_value(nal->buffer + buffer_on,
			&size);
#ifdef DEBUG_AVS
...
#endif
		buffer_on += size;
		payload_size = avs_read_sei_value(nal->buffer + buffer_on,
			&size);
...
static bool LoadNal (nal_reader_t *nal)
{
	...
	// find start code
	uint32_t start;
	if (avs_is_start_code(nal->buffer) == false) {
		start = avs_find_next_start_code(nal->buffer,
			nal->buffer_size);
		RefreshReader(nal, nal->buffer_on);
	}

	if (avs_is_start_code(nal->buffer + 4) == false)   //HuoLongshe_20071104
	{
		while ((start = avs_find_next_start_code(nal->buffer + 4,
			nal->buffer_size - 4)) == 0) {
	...
	{
		nal->buffer_on = 4;  //HuoLongshe_20071104
	}

	return true;
}

MP4TrackId AVSCreator (MP4FileHandle mp4File, 
					   FILE* inFile, FILE* datFile)//Lirh20071113
{
	int stateFirstI = 0;
    bool rc;
	int iTime; //Lirh20071113
    ...
    uint8_t AvsProfileIndication = 0;
    uint8_t AvsLevelIndication = 0;
    // first thing we need to do is read the headers
    bool have_seq = false;
    uint8_t nal_type;
    nal_reader_t nal;
    avs_decode_t avs_dec;
    MP4Timestamp lastTime = 0, thisTime =0 ,backwardpoint = 0;//Lirh20071113
	...
	{
	fprintf(stderr, "%s: Must specify frame rate when reading AVS files", ProgName);
	return MP4_INVALID_TRACK_ID;
	}
	*/
    while (have_seq == false) {
		if (LoadNal(&nal) == false) {
			fprintf(stderr, "%s: Could not find avs parameter header\n", ProgName);
			return MP4_INVALID_TRACK_ID;
		}
		nal_type = avs_nal_unit_type(nal.buffer);
		
		if (nal_type == AVS_NAL_TYPE_SEQ_HEADER) {
			have_seq = true;
			uint32_t offset = 3;
			// skip the nal type byte
			AvsProfileIndication = nal.buffer[offset + 1];
			AvsLevelIndication = nal.buffer[offset + 2];
			if (avs_read_seq_header(nal.buffer, nal.buffer_on, &avs_dec) == -1) {
				fprintf(stderr, "%s: Could not decode avs Sequence header\n", ProgName);
				return MP4_INVALID_TRACK_ID;
			}
		}
	...
	if (VideoFrameRate == 0.0) 
	{
		VideoFrameRate = avs_dec.frame_rate;
	}
    ...
    MP4TrackId trackId;
    trackId =
		MP4AddAVSVideoTrack(mp4File,
		Mp4TimeScale,
		mp4FrameDuration,
		avs_dec.horizontal_size,
		avs_dec.vertical_size,
		AvsProfileIndication,
		AvsLevelIndication,
		3);
    ...
    //bool first = true;  //HuoLongshe_del_20071105
    ...
    memset(&avs_dec, 0, sizeof(avs_dec));
    DpbInit1(&avs_dpb);
	
    while ( LoadNal(&nal) != false ) {
		uint32_t header_size;
		header_size = 3;
		avs_dec.last_nal_unit_type = avs_dec.nal_unit_type;
		bool boundary = avs_detect_boundary(nal.buffer, 
			nal.buffer_on,
			&avs_dec);

		   //HuoLongshe_add_20071105
		if (((avs_dec.last_nal_unit_type >= AVS_NAL_TYPE_I_PIC_HEADER) && (avs_dec.last_nal_unit_type <= AVS_NAL_TYPE_B_SLICE))
			&& (avs_dec.nal_unit_type != AVS_NAL_TYPE_I_SLICE)) 
		{
		//if (boundary && first == false) {   //HuoLongshe_del_20071105
...
//-------------------------------------------------------------//Lirh20071207
				...
				{
					fread(&iTime, 4, 1, datFile);

					thisTime = (MP4Timestamp)iTime * Mp4TimeScale / 1000;
					dur = thisTime - lastTime;
//					printf("%d \n", dur);

					if((thisTime < lastTime)|| backward==true)
					{
						if(thisTime < lastTime)
						{
							backwardpoint=lastTime;
							backward=true;
							dur = 0;
						}
						else
							if(thisTime > backwardpoint)
							{
								dur = thisTime-backwardpoint;
								backward=false;
							}
							else
								dur = 0;
...
//				dur=3600;
				...
				/*     //HuoLongshe_del_20071105
				if (((avs_dec.last_nal_unit_type < AVS_NAL_TYPE_I_PIC_HEADER)
					||(avs_dec.last_nal_unit_type > AVS_NAL_TYPE_B_SLICE)))
				{
					dur = 0;
				}
				*/  
				...
				//HuoLongshe_begin
				//下面的修改仅适用于IPBBPBBPBB...PBBIBBPBBPBB...类型的序列，而且帧率固定的情形
				MP4Duration RendingOffset = 0;
				if (avs_dec.last_nal_unit_type == AVS_NAL_TYPE_I_PIC_HEADER)
				{
					if (stateFirstI == 0)
					{
				...
				else if  (avs_dec.last_nal_unit_type == AVS_NAL_TYPE_I_SLICE)
				{
					if (stateFirstI == 1)
					{
						RendingOffset = dur;
				...
				else if  ((avs_dec.last_nal_unit_type == AVS_NAL_TYPE_P_PIC_HEADER) || (avs_dec.last_nal_unit_type == AVS_NAL_TYPE_P_SLICE))
				{
					RendingOffset = 3 * dur;
				}
				else
				...
				rc = MP4WriteSample(mp4File, 
					...
					nal_buffer_size,
					dur,	
					0, //RendingOffset,  //HuoLongshe //由于播放器不支持，暂且不用正确的PTS
					nal_is_sync);
				...
				sampleId++;
				DpbAdd1( &avs_dpb, poc, slice_is_idr );
				nal_is_sync = false;
#ifdef DEBUG_AVS
		...
		if (Verbosity & MP4_DETAILS_SAMPLE) {
			printf("AVS type %x size %u\n",
				avs_dec.nal_unit_type, nal.buffer_on);
		}
		if (avs_nal_unit_type_is_slice(avs_dec.nal_unit_type)) {
			copy_nal_to_buffer = true;
			slice_is_idr = avs_dec.pic_is_idr;
			poc = avs_dec.pic_order_cnt;
			
			nal_is_sync = avs_picture_is_idr(&avs_dec);
		} else {
			switch (avs_dec.nal_unit_type) {
			case AVS_NAL_TYPE_SEQ_HEADER:
				// doesn't get added to sample buffer
				// remove header
				copy_nal_to_buffer = true;
				MP4AddAVSSequenceHeader(mp4File, trackId, 
					nal.buffer + header_size, 
					nal.buffer_on - header_size);
				break;
			case AVS_NAL_TYPE_P_PIC_HEADER:
			case AVS_NAL_TYPE_B_PIC_HEADER:
				avs_dec.pic_is_idr = false;
				copy_nal_to_buffer = true;
				break;
			case AVS_NAL_TYPE_I_PIC_HEADER:
				avs_dec.pic_is_idr = true;
				copy_nal_to_buffer = true;
				nal_is_sync = true;
				break;
			default:
		...
		if (copy_nal_to_buffer) {
			uint32_t to_write;
			to_write = nal.buffer_on - header_size + 1;  //减去0x000001，加上NALU Type
			...
			nal_buffer[nal_buffer_size] = (to_write >> 24) & 0xff;  //大小不包含size字段本身（参见AVS-P9）
			...
			nal_buffer[nal_buffer_size + 4] = avs_dec.nal_unit_type;
			if (avs_dec.nal_unit_type == AVS_NAL_TYPE_I_SLICE)
			{
				if (avs_dec.picture_coding_type == AVS_TYPE_I)
				{
					nal_buffer[nal_buffer_size + 4] = AVS_NAL_TYPE_I_SLICE;
				}
				else if (avs_dec.picture_coding_type == AVS_TYPE_P)
				{
					nal_buffer[nal_buffer_size + 4] = AVS_NAL_TYPE_P_SLICE;
					avs_dec.nal_unit_type = AVS_NAL_TYPE_P_SLICE; 
					//HuoLongshe: 对于slice，原来的nal_unit_type统一赋值为AVS_NAL_TYPE_I_SLICE，这里把它改正确
				}
				else if (avs_dec.picture_coding_type == AVS_TYPE_B)
				{
					nal_buffer[nal_buffer_size + 4] = AVS_NAL_TYPE_B_SLICE;
					avs_dec.nal_unit_type = AVS_NAL_TYPE_B_SLICE;
					//HuoLongshe: 对于slice，原来的nal_unit_type统一赋值为AVS_NAL_TYPE_I_SLICE，这里把它改正确
				}
			}
			
			memcpy(nal_buffer + nal_buffer_size + 4 + 1,  //4: NAL size, 1: NAL Type
				nal.buffer + header_size,
				to_write - 1);  // -1: NAL Type已经拷贝
#ifdef DEBUG_AVS
    ...
    if (nal_buffer_size != 0) 
	{
		...
		MP4Duration dur;
		dur = thisTime - lastTime;
		dur = thisTime > lastTime ? dur : 0;//Lirh20071207
		
		MP4Duration RendingOffset = 0;
		if  ((avs_dec.last_nal_unit_type == AVS_NAL_TYPE_I_PIC_HEADER) || (avs_dec.last_nal_unit_type == AVS_NAL_TYPE_I_SLICE))
		{
			RendingOffset = 3 * dur;
		}
		else if  ((avs_dec.last_nal_unit_type == AVS_NAL_TYPE_P_PIC_HEADER) || (avs_dec.last_nal_unit_type == AVS_NAL_TYPE_P_SLICE))
...
//		printf("%d \n", dur);
		...
		DpbAdd1(&avs_dpb, avs_dec.pic_order_cnt, slice_is_idr);
    }
	
    DpbFlush1(&avs_dpb);
	
	/* HuoLongshe_del
    if (avs_dpb.dpb.size_min > 0) {
		unsigned int ix;
		
		for (ix = 0; ix < samplesWritten; ix++) 
		{
			const int offset = DpbFrameOffset1(&avs_dpb, ix);
