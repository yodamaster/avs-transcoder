...
extern "C" MP4TrackId MP4AddVideoTrack(										//****MP4AddVideoTrack()****
	MP4FileHandle hFile, 
	u_int32_t timeScale, 
	MP4Duration sampleDuration,
	u_int16_t width, 
...
extern "C" MP4TrackId MP4AddAVSMVideoTrack(
										   MP4FileHandle hFile, 
										   u_int32_t timeScale, 
										   MP4Duration sampleDuration,
										   u_int16_t width, 
										   ...
										   uint8_t sampleLenFieldSizeMinusOne)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			...
			return pFile->AddAVSMVideoTrack(timeScale, 
				sampleDuration, 
				width, 
				height, 
...
extern "C" bool MP4AddAVSMSequenceParameterSet (MP4FileHandle hFile,
												MP4TrackId trackId,
												const uint8_t *pSequence,
												uint16_t sequenceLen)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			...
			return pFile->AddAVSMSequenceParameterSet(trackId,
				pSequence,
				sequenceLen);
		}
		catch (MP4Error* e) {
...
extern "C" bool MP4AddAVSMPictureParameterSet (MP4FileHandle hFile,
											   MP4TrackId trackId,
											   const uint8_t *pPict,
											   uint16_t pictLen)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			...
			return pFile->AddAVSMPictureParameterSet(trackId,
				pPict,
				pictLen);
		}
		catch (MP4Error* e) {
...
extern "C" MP4TrackId MP4AddAVSVideoTrack(
										   MP4FileHandle hFile, 
										   u_int32_t timeScale, 
										   MP4Duration sampleDuration,
										   u_int16_t width, 
										   ...
										   uint8_t sampleLenFieldSizeMinusOne)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			...
			return pFile->AddAVSVideoTrack(timeScale, 
				sampleDuration, 
				width, 
				height, 
...
extern "C" bool MP4AddAVSSequenceHeader(MP4FileHandle hFile,
												MP4TrackId trackId,
												const uint8_t *pSequence,
												uint16_t sequenceLen)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			...
			return pFile->AddAVSSequenceHeader(trackId,
				pSequence,
				sequenceLen);
		}
		catch (MP4Error* e) {
...
extern "C" bool MP4GetTrackAVSMProfileLevel (MP4FileHandle hFile, 
					     MP4TrackId trackId,
					     uint8_t *pProfile,
					     uint8_t *pLevel)
{
  if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
    try {
      ((MP4File*)hFile)->GetTrackAVSMProfileLevel(trackId, pProfile, pLevel);
      return true;
    }
    catch (MP4Error* e) {
      PRINT_ERROR(e);
...
extern "C" bool MP4GetTrackAVSMSeqPictHeaders (MP4FileHandle hFile, 
											   MP4TrackId trackId,
											   uint8_t ***pSeqHeader,
											   uint32_t **pSeqHeaderSize,
											   uint8_t ***pPictHeader,
											   uint32_t **pPictHeaderSize)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			((MP4File*)hFile)->GetTrackAVSMSeqPictHeaders(trackId, 
				pSeqHeader,
				pSeqHeaderSize,
				pPictHeader,
				pPictHeaderSize);
...
extern "C" bool MP4GetTrackAVSMLengthSize (MP4FileHandle hFile, 
										   MP4TrackId trackId,
										   uint32_t *pLength)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			((MP4File*)hFile)->GetTrackAVSMLengthSize(trackId, 
				pLength);
			return true;
		}
		catch (MP4Error* e) {
...
//added for AVS2.
extern "C" bool MP4GetTrackAVSProfileLevel (MP4FileHandle hFile, 
											 MP4TrackId trackId,
											 uint8_t *pProfile,
											 uint8_t *pLevel)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			((MP4File*)hFile)->GetTrackAVSProfileLevel(trackId, pProfile, pLevel);
			return true;
		}
		catch (MP4Error* e) {
			PRINT_ERROR(e);
...
extern "C" bool MP4GetTrackAVSSeqHeaders (MP4FileHandle hFile, 
											   MP4TrackId trackId,
											   uint8_t ***pSeqHeader,
											   uint32_t **pSeqHeaderSize)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			((MP4File*)hFile)->GetTrackAVSSeqHeaders(trackId, 
				pSeqHeader,
				pSeqHeaderSize);
			return true;
		}
...
extern "C" bool MP4GetTrackAVSLengthSize (MP4FileHandle hFile, 
										   MP4TrackId trackId,
										   uint32_t *pLength)
{
	if (MP4_IS_VALID_FILE_HANDLE(hFile)) {
		try {
			((MP4File*)hFile)->GetTrackAVSLengthSize(trackId, 
				pLength);
			return true;
		}
		catch (MP4Error* e) {
