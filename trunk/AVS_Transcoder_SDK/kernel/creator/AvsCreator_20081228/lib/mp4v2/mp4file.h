...
public: /* equivalent to MP4 library API */
	...
	MP4TrackId AddAVSMVideoTrack(
		...
		uint8_t sampleLenFieldSizeMinusOne);
	bool AddAVSMSequenceParameterSet(MP4TrackId trackId,
		const uint8_t *pSequence,
		uint16_t sequenceLen);
	bool AddAVSMPictureParameterSet(MP4TrackId trackId,
		const uint8_t *pPicture,
		uint16_t pictureLen);

	MP4TrackId AddAVSVideoTrack(
		u_int32_t timeScale,
		MP4Duration sampleDuration,
		u_int16_t width,
		u_int16_t height,
		...
		uint8_t sampleLenFieldSizeMinusOne);
	bool AddAVSSequenceHeader(MP4TrackId trackId,
		const uint8_t *pSequence,
		uint16_t sequenceLen);
	...
	void GetTrackAVSMProfileLevel(MP4TrackId trackId,
		uint8_t *pProfile,
		uint8_t *pLevel);	
	bool GetTrackAVSMSeqPictHeaders(MP4TrackId trackId, 
		uint8_t ***pSeqHeader,
		uint32_t **pSeqHeaderSize,
		uint8_t ***pPictHeader,
		uint32_t **pPictHeaderSize);
	void GetTrackAVSMLengthSize(MP4TrackId, 
				    uint32_t *pLength);
	
	//added for AVS2.
	void GetTrackAVSProfileLevel(MP4TrackId trackId,
		uint8_t *pProfile,
		uint8_t *pLevel);	
	bool GetTrackAVSSeqHeaders(MP4TrackId trackId, 
		uint8_t ***pSeqHeader,
		uint32_t **pSeqHeaderSize);
	void GetTrackAVSLengthSize(MP4TrackId, 
				    uint32_t *pLength);
