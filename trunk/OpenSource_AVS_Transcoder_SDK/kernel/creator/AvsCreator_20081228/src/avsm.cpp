...
void DpbClean1( avsm_dpb_t *p )
{
	free( p->frame );
}
static void DpbUpdate( avsm_dpb_t *p, int is_forced )
{
	int i;
	int pos;
...
void DpbFlush1( avsm_dpb_t *p )
{
	while (p->dpb.cnt > 0)
		DpbUpdate( p, true );
}

void DpbAdd1( avsm_dpb_t *p, int poc, int is_idr )
{
	if (is_idr)
		DpbFlush1( p );
...
int DpbFrameOffset1( avsm_dpb_t *p, int idx )
{
	if (idx >= p->cnt)
		return 0;
	if (p->frame[idx] < 0)
										...
										uint32_t header_size)
{
	...
	while (buffer_on < nal->buffer_on) {
		...
		start = buffer_on;
		payload_type = avsm_read_sei_value(nal->buffer + buffer_on,
			&size);
#ifdef DEBUG_AVSM
...
#endif
		buffer_on += size;
		payload_size = avsm_read_sei_value(nal->buffer + buffer_on,
			&size);
...
static bool LoadNal (nal_reader_t *nal)
{
	...
	// find start code
	uint32_t start;
	if (avsm_is_start_code(nal->buffer) == false) {
		start = avsm_find_next_start_code(nal->buffer,
			nal->buffer_size);
		RefreshReader(nal, nal->buffer_on);
	}
	while ((start = avsm_find_next_start_code(nal->buffer + 4,
		nal->buffer_size - 4)) == 0) {
...
MP4TrackId AVSMCreator (MP4FileHandle mp4File, 
						FILE* inFile)
{
    bool rc;
    ...
    uint8_t AvsmProfileIndication = 0;
    uint8_t profile_compat = 0;
    uint8_t AvsmLevelIndication = 0;
    // first thing we need to do is read the headers
    bool have_seq = false;
    uint8_t nal_type;
    nal_reader_t nal;
    avsm_decode_t avsm_dec;
	...
	fprintf(stderr, "%s: Must specify frame rate when reading AVSM files", 
	ProgName);
	return MP4_INVALID_TRACK_ID;
}*/
    while (have_seq == false) {
		if (LoadNal(&nal) == false) {
			fprintf(stderr, "%s: Could not find avsm parameter set\n", ProgName);
			return MP4_INVALID_TRACK_ID;
		}
		nal_type = avsm_nal_unit_type(nal.buffer);
		
		if (nal_type == AVSM_NAL_TYPE_SEQ_PARAM) {
			have_seq = true;
			uint32_t offset;
			if (nal.buffer[2] == 1) offset = 3;
			else offset = 4;
			// skip the nal type byte
			AvsmProfileIndication = nal.buffer[offset + 1];
			AvsmLevelIndication = nal.buffer[offset + 2];
			if (avsm_read_seq_info(nal.buffer, nal.buffer_on, &avsm_dec) == -1) {
				fprintf(stderr, "%s: Could not decode avsm Sequence header\n", ProgName);
				return MP4_INVALID_TRACK_ID;
			}
		}
	...
	if (VideoFrameRate == 0.0) 
	{
		VideoFrameRate = 90000 / avsm_dec.delta_time_picture_distance_1;
	}
    ...
    memset(&avsm_dec, 0, sizeof(avsm_dec));
    DpbInit1(&avsm_dpb);
	
    while ( LoadNal(&nal) != false ) {
		uint32_t header_size;
		header_size = nal.buffer[2] == 1 ? 3 : 4;
		bool boundary = avsm_detect_boundary(nal.buffer, 
			nal.buffer_on,
			&avsm_dec);
		if (boundary && first == false) {
			...
			if (nal_buffer_size != 0) {
				...
				sampleId++;
				DpbAdd1( &avsm_dpb, poc, slice_is_idr );
				nal_is_sync = false;
#ifdef DEBUG_AVSM
		...
		if (Verbosity & MP4_DETAILS_SAMPLE) {
			printf("AVSM type %x size %u\n",
				avsm_dec.nal_unit_type, nal.buffer_on);
		}
		if (avsm_nal_unit_type_is_slice(avsm_dec.nal_unit_type)) {
			copy_nal_to_buffer = true;
			slice_is_idr = avsm_dec.pic_is_idr;
			poc = avsm_dec.pic_order_cnt;
			
			nal_is_sync = avsm_picture_is_idr(&avsm_dec);
		} else {
			switch (avsm_dec.nal_unit_type) {
			case AVSM_NAL_TYPE_SEQ_PARAM:
				// doesn't get added to sample buffer
				// remove header
				
				MP4AddAVSMSequenceParameterSet(mp4File, trackId, 
					nal.buffer + header_size, 
					nal.buffer_on - header_size);
				break;
			case AVSM_NAL_TYPE_PIC_PARAM:
				// doesn't get added to sample buffer
				
				MP4AddAVSMPictureParameterSet(mp4File, trackId, 
					nal.buffer + header_size, 
					nal.buffer_on - header_size);
				break;
			case AVSM_NAL_TYPE_NON_IDR_PIC_HEADER:
				avsm_dec.pic_is_idr = false;
				copy_nal_to_buffer = true;
				break;
			case AVSM_NAL_TYPE_IDR_PIC_HEADER:
				avsm_dec.pic_is_idr = true;
				copy_nal_to_buffer = true;
				break;
    ...
    if (nal_buffer_size != 0) {
		...
		DpbAdd1(&avsm_dpb, avsm_dec.pic_order_cnt, slice_is_idr);
    }
	
    DpbFlush1(&avsm_dpb);
	
    if (avsm_dpb.dpb.size_min > 0) {
		unsigned int ix;
		
		for (ix = 0; ix < samplesWritten; ix++) {;
		const int offset = DpbFrameOffset1(&avsm_dpb, ix);
