	...
	const char *videoType)
{
	...
	AddChildAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd"), videoType);		//videoType=mp4v
...
MP4TrackId MP4File::AddAVSMVideoTrack(
	u_int32_t timeScale, 
	MP4Duration sampleDuration, 
	u_int16_t width, 
	u_int16_t height, 
	uint8_t AvsmProfileIndication,
	uint8_t profile_compat,
	uint8_t AvsmLevelIndication,
	uint8_t sampleLenFieldSizeMinusOne)
{
  MP4TrackId trackId = AddVideoTrackDefault(timeScale, 
					    sampleDuration,
	...
	SetTrackIntegerProperty(trackId, 
		"mdia.minf.stbl.stsd.avs1.width", width);
	SetTrackIntegerProperty(trackId, 
		"mdia.minf.stbl.stsd.avs1.height", height);
	...
	*/
	SetTrackIntegerProperty(trackId,
				"mdia.minf.stbl.stsd.avs1.avsc.AvsmProfileIndication",
				AvsmProfileIndication);
	SetTrackIntegerProperty(trackId,
				"mdia.minf.stbl.stsd.avs1.avsc.AvsmLevelIndication",
				AvsmLevelIndication);
	SetTrackIntegerProperty(trackId,
				"mdia.minf.stbl.stsd.avs1.avsc.lengthSizeMinusOne",
				sampleLenFieldSizeMinusOne);
...
bool MP4File::AddAVSMSequenceParameterSet (MP4TrackId trackId,
					   const uint8_t *pSequence,
					   uint16_t sequenceLen)
{
  MP4Atom *avscAtom = 
    FindAtom(MakeTrackName(trackId,
			   "mdia.minf.stbl.stsd.avs1.avsc"));
  MP4BitfieldProperty *pCount;
  MP4Integer16Property *pLength;
  MP4BytesProperty *pUnit;
  if ((avscAtom->FindProperty("avsc.numOfSequenceParameterSets",
			     (MP4Property **)&pCount) == false) ||
      (avscAtom->FindProperty("avsc.sequenceEntries.sequenceParameterSetLength",
			      (MP4Property **)&pLength) == false) ||
      (avscAtom->FindProperty("avsc.sequenceEntries.sequenceParameterSetNALUnit",
			      (MP4Property **)&pUnit) == false)) {
    VERBOSE_ERROR(m_verbosity, WARNING("Could not find avsc properties"));
    return false;
  }
  uint32_t count = pCount->GetValue();
...
bool MP4File::AddAVSMPictureParameterSet (MP4TrackId trackId,
					  const uint8_t *pPict,
					  uint16_t pictLen)
{
  MP4Atom *avscAtom = 
  ...
  MP4Integer16Property *pLength;
  MP4BytesProperty *pUnit;
  if ((avscAtom->FindProperty("avsc.numOfPictureParameterSets",
			     (MP4Property **)&pCount) == false) ||
      (avscAtom->FindProperty("avsc.pictureEntries.pictureParameterSetLength",
			      (MP4Property **)&pLength) == false) ||
      (avscAtom->FindProperty("avsc.pictureEntries.pictureParameterSetNALUnit",
			      (MP4Property **)&pUnit) == false)) {
    VERBOSE_ERROR(m_verbosity, 
		  WARNING("Could not find avsc picture table properties"));
    return false;
  }
  uint32_t count = pCount->GetValue();
...
MP4TrackId MP4File::AddAVSVideoTrack(
	u_int32_t timeScale, 
	MP4Duration sampleDuration, 
	u_int16_t width, 
	u_int16_t height, 
	uint8_t AvsProfileIndication,
	uint8_t AvsLevelIndication,
	uint8_t sampleLenFieldSizeMinusOne)
{
  MP4TrackId trackId = AddVideoTrackDefault(timeScale, 
					    sampleDuration,
	...
	SetTrackIntegerProperty(trackId, 
		"mdia.minf.stbl.stsd.avs2.width", width);
	SetTrackIntegerProperty(trackId, 
		"mdia.minf.stbl.stsd.avs2.height", height);

	/* shouldn't need this
	AddChildAtom(MakeTrackName(trackId,
				   "mdia.minf.stbl.stsd.avs2"),
		     "avss");
	*/
	SetTrackIntegerProperty(trackId,
				"mdia.minf.stbl.stsd.avs2.avss.AvsProfileIndication",
				AvsProfileIndication);
	SetTrackIntegerProperty(trackId,
				"mdia.minf.stbl.stsd.avs2.avss.AvsLevelIndication",
				AvsLevelIndication);
	SetTrackIntegerProperty(trackId,
				"mdia.minf.stbl.stsd.avs2.avss.lengthSizeMinusOne",
				sampleLenFieldSizeMinusOne);
...
bool MP4File::AddAVSSequenceHeader (MP4TrackId trackId,
					   const uint8_t *pSequence,
					   uint16_t sequenceLen)
{
	MP4Atom *avssAtom = 
		FindAtom(MakeTrackName(trackId,
		"mdia.minf.stbl.stsd.avs2.avss"));
	MP4BitfieldProperty *pCount;
	MP4Integer16Property *pLength;
	MP4BytesProperty *pUnit;
	if ((avssAtom->FindProperty("avss.numOfSequenceHeader",
			     (MP4Property **)&pCount) == false) ||
				 (avssAtom->FindProperty("avss.sequenceEntries.SequenceHeaderLength",
				 (MP4Property **)&pLength) == false) ||
				 (avssAtom->FindProperty("avss.sequenceEntries.SequenceHeaderNALUnit",
				 (MP4Property **)&pUnit) == false)) {
		VERBOSE_ERROR(m_verbosity, WARNING("Could not find avss properties"));
		return false;
	}
	uint32_t count = pCount->GetValue();
...
//Aug 1, MLT: added for AVS1.
void MP4File::GetTrackAVSMProfileLevel (MP4TrackId trackId, 
					uint8_t *pProfile, 
					uint8_t *pLevel)
{
  *pProfile = 
...
void MP4File::GetTrackAVSMLengthSize (MP4TrackId trackId, 
				      uint32_t *pLength)
{
  *pLength = 1 + 
    GetTrackIntegerProperty(trackId, 
...
bool MP4File::GetTrackAVSMSeqPictHeaders (MP4TrackId trackId,
										  uint8_t ***pppSeqHeader,
										  uint32_t **ppSeqHeaderSize,
										  uint8_t ***pppPictHeader,
										  uint32_t **ppPictHeaderSize)
...
//added for AVS2.
void MP4File::GetTrackAVSProfileLevel (MP4TrackId trackId, 
										uint8_t *pProfile, 
										uint8_t *pLevel)
{
	*pProfile = 
...
void MP4File::GetTrackAVSLengthSize (MP4TrackId trackId, 
									  uint32_t *pLength)
{
	*pLength = 1 + 
		GetTrackIntegerProperty(trackId, 
...
bool MP4File::GetTrackAVSSeqHeaders (MP4TrackId trackId,
										  uint8_t ***pppSeqHeader,
										  uint32_t **ppSeqHeaderSize)
{
	uint32_t count;
	...
	MP4Atom *avssAtom = 
		...
		"mdia.minf.stbl.stsd.avs2.avss"));
	MP4BitfieldProperty *pSeqCount;
	MP4IntegerProperty *pSeqLen, *pPictCount;
	MP4BytesProperty *pSeqVal;
	if ((avssAtom->FindProperty("avss.numOfSequenceHeader",
			     (MP4Property **)&pSeqCount) == false) ||
				 (avssAtom->FindProperty("avss.sequenceEntries.SequenceHeaderLength",
				 (MP4Property **)&pSeqLen) == false) ||
...
{
	ProtectWriteOperation("MP4AddRtpHint");		//È·¶¨¿ÉÐ´
