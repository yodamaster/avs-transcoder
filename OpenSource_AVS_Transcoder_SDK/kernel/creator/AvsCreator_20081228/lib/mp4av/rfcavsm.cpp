						  ...
						  MP4TrackId mediaTrackId)
{
 ...
 static uint32_t avsm_get_nal_size (uint8_t *pData,
	 uint32_t sizeLength)
 {
	 if (sizeLength == 1) {
		 return *pData;
 ...
 static uint8_t avsm_get_sample_nal_type (uint8_t *pSampleBuffer, 
	 uint32_t sampleSize,
	 uint32_t sizeLength)
 {
	 ...
	 while (offset < sampleSize) {
		 nal_type = pSampleBuffer[sizeLength] & 0x1f;
		 uint32_t add;
		 
		 if (avsm_nal_unit_type_is_slice(nal_type)) {
			 return nal_type;
		 }
		 add = avsm_get_nal_size(pSampleBuffer, sizeLength) + sizeLength;
		 offset += add;
		 pSampleBuffer += add;
	 }
	 */
