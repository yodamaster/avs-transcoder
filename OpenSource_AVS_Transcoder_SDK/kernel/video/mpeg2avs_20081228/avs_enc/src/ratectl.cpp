...
void c_avs_enc:: rc_init_seq()
{
   ...
   // xzhao 20081022
   //frame_rate = (int_32_t)(img->framerate);
   ...
   /*basic unit layer rate control*/
	...
	//xzhao 20081108
    RC_MAX_QUANT = 40;  // clipping  50 for LBC    35 for HD
    RC_MIN_QUANT = 26;//clipping     36 for LBC    22 for HD
...
void c_avs_enc::rc_init_GOP(int_32_t np, int_32_t nb)
{
 ...
 /*compute the total number of bits for the current GOP*/ 
  ...
  OverDuantQp=(int_32_t)(8*OverBits/AllocatedBits+0.5);
  GOPOverdue=FALSE;
...
/*basic unit layer rate control*/
  ...
  if((img->type==INTER_IMG)&&(img->BasicUnit<img->Frame_Total_Number_MB)\
    &&(img->FieldControl==1))
  {
  ...
  if(img->type==INTER_IMG)
  {
    ...
    else
    {
      ...
      /* compute the average number of header bits*/
        ...
        if(CodedBasicUnit>0)
        {
          PAveHeaderBits1=(int_32_t)(1.0*(PAveHeaderBits1*(CodedBasicUnit-1)+\
            +img->NumberofBasicUnitHeaderBits)/CodedBasicUnit+0.5);
          if(PAveHeaderBits3==0)
            PAveHeaderBits2=PAveHeaderBits1;
          else
            PAveHeaderBits2=(int_32_t)(1.0*(PAveHeaderBits1*CodedBasicUnit+\
...
/*compute the size of window*/
  ...
  for (i = 0; i < (int_32_t) n_windowSize; i++) {
